...
def add_sc_spherical_coords_to_moments(moments_df, spice_kernel_dir='data/SO/SPICE'):
    """
    Add Solar Orbiter's heliocentric spherical coordinates to a moments dataframe.
    ...
    spice_kernel_dir : str
        Directory containing the local Solar Orbiter SPICE kernels.
    ...
    kernel_dir = Path(spice_kernel_dir)
    kernel_names = (
        'naif0012.tls',
        'pck00010.tpc',
        'solo_ANC_soc-sci-fk_V08.tf',
        'solo_ANC_soc-sclk_20250803_V01.tsc',
        'de421.bsp',
        'solo_ANC_soc-orbit_20200210-20301120_L022_V1_00464_V02.bsp',
    )
    kernels = [kernel_dir / name for name in kernel_names]
    missing = [str(path) for path in kernels if not path.exists()]
    if missing:
        raise FileNotFoundError(f"Missing SPICE kernel(s): {', '.join(missing)}")
    ...
    try:
        ...
        coordinates = []
        for timestamp in out['Time']:
            et = spice.str2et(timestamp.strftime('%Y-%m-%dT%H:%M:%S.%f'))
            position, _ = spice.spkezr('SOLO', et, 'SOLO_HCI', 'NONE', 'Sun')
            x, y, z = position[:3]
            radius = np.linalg.norm(position[:3]) / (const.au * 1e-3)
            theta = np.degrees(np.arccos(np.clip(z / np.linalg.norm(position[:3]), -1.0, 1.0)))
            phi = np.degrees(np.arctan2(y, x))
            coordinates.append((radius, theta, phi))
    finally:
        for kernel in reversed(loaded):
            spice.unload(str(kernel))

    out[['R_SC', 'Theta_SC', 'Phi_SC']] = np.asarray(coordinates)
