...
def all_process(idx_time, Protons_initial, Alphas_initial, vdf_cdffile, grnd_cdffile, mag_cdffile, one_particle_noise_level, result_path):
    ...
    def remove_alpha_low_speed(f_alpha, co_type):
        # Sometimes there are weired low-speed alphas (Like a strong peak caused by the overlap), they might ruin the computing of moments.
    ...
    while alpha_dividing_idx > dividing_idx:
        print('above ' + str(alpha_dividing_idx) + ' set to 0') 
        f_alpha_tied[:, :, alpha_dividing_idx:] = 0
        idx_new = remove_alpha_low_speed(f_alpha_tied, 'Tied')
        if idx_new == alpha_dividing_idx:
            break
        print(idx_new)
        alpha_dividing_idx = idx_new
    ...
    #_, which_one, Protons_current, Alphas_current, Scores_current = options[0]
...
def main():
    ...
    t_start = datetime(2022, 10, 7, 7, 0, 5)
    hhmmss = (t_start - timedelta(seconds=4)).strftime("%H%M%S")
    t_end = datetime(2022, 10, 7, 9, 0, 0)
