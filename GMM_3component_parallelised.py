...
def parallelised_all_process(idx_time_list, initial_means, n_processes):
    ...
    return results[-1]  # only return the one we needed for the initial value of the next block.


def main():
    # t start should be the the time of the initial separation + 4s.
    t_start = datetime(2022, 10, 23, 5, 0, 4)
    hhmmss = (t_start - timedelta(seconds=4)).strftime("%H%M%S")
    t_end = datetime(2022, 10, 23, 5, 10, 0)
