...
def cal_bulk_velocity_Spherical(SW_spherical, component=None):
    ...
    vx_ele = np.trapezoid(vx_kernal, x=np.radians(elevation), axis=1)
    vy_ele = np.trapezoid(vy_kernal, x=np.radians(elevation), axis=1)
    vz_ele = np.trapezoid(vz_kernal, x=np.radians(elevation), axis=1)

    vx_ele_energy = -np.trapezoid(vx_ele, x=velocity, axis=1)
    vy_ele_energy = -np.trapezoid(vy_ele, x=velocity, axis=1)
    vz_ele_energy = -np.trapezoid(vz_ele, x=velocity, axis=1)

    vx_all = np.trapezoid(vx_ele_energy, x=np.radians(azimuth), axis=0)
    vy_all = np.trapezoid(vy_ele_energy, x=np.radians(azimuth), axis=0)
    vz_all = np.trapezoid(vz_ele_energy, x=np.radians(azimuth), axis=0)
...
def cal_pressure_tensor_Spherical(SW_spherical, component=None):
    ...
    def integrate_pressure(kernal):
        int_ele = np.trapezoid(kernal, x=np.radians(elevation), axis=1)
        int_energy = -np.trapezoid(int_ele, x=velocity, axis=1)
        return np.trapezoid(int_energy, x=np.radians(azimuth), axis=0)
...
def cal_density_2D(SW_2D, component=None):
    ...
    int_para = np.trapezoid(int_kernal, x=Vpara, axis=0)
    int_perp = np.trapezoid(int_para, x=Vperp, axis=0)
