...
The pipeline is tested with **Python 3.10** and pinned package versions. The easiest way to get a working setup is to create the conda environment provided in this repository:
...
This installs every required package with the exact versions the pipeline was tested with (NumPy 1.26, SciPy 1.15, Matplotlib 3.10, scikit-learn 1.6, pandas 2.3, ...). It works on Linux, macOS, and Windows.

The moments calculation uses Solar Orbiter SPICE kernels to calculate the spacecraft position. Download them once from the official ESA archive:

```bash
python download_Spice_kernel.py
```

The kernels are stored locally in `data/SO/SPICE/` and are not downloaded once per day or once per worker. The dataset should be cited as [ESA SPICE Service, Solar Orbiter Operational SPICE Kernel Dataset](https://doi.org/10.5270/esa-kt1577e).

If you prefer to build your own environment, the required packages are:
...
scikit-learn
astropy
h5py, netcdf4, pytables
sunpy, sunpy-soar, spacepy, spiceypy, cdflib   # data download and SPICE coordinates
seaborn, xarray                      # analysis & plotting scripts
jupyter                              # notebooks
```

> ⚠️ **Important:** the code is tested with **NumPy 1.26** and is *not* verified under NumPy 2.x. If you set up your own environment, keep `numpy<2`.
...
```bash
# 0. Create the environment (see "Before You Run")
conda env create -f environment.yml && conda activate GMM_env
...
├── environment.yml                # Pinned conda environment (see "Before You Run")
│
...
All available in the `GMM_env` conda environment — create it with `conda env create -f environment.yml` (see [Before You Run](#before-you-run)).
