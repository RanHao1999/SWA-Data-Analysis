...
def OneParticleNoiseLevel(count_cdffile, vdffile):
    ...
    if len(vdf_epochs) == 0 or len(count_epochs) == 0:
        return np.nan
    ...
    match_indices = np.clip(match_indices, 0, len(count_epochs) - 1) # Keep rare edge cases inside the valid range
...
def cal_overlap(protons, alphas):
    ...
    min_vdf = np.minimum(vdf_proton, vdf_alpha)
    overlap_alpha = np.sum(min_vdf) / np.sum(vdf_alpha)
    overlap_proton = np.sum(min_vdf) / np.sum(vdf_proton)
    
    return overlap_alpha, overlap_proton
...
def add_sc_spherical_coords_to_moments(moments_df, timestamps_npy_path, sph_coords_npz_path):
    """
    Add spacecraft spherical coordinates to a moments dataframe.

    Parameters:
    -----------
    moments_df : pd.DataFrame
        Moments dataframe indexed by time.
    timestamps_npy_path : str
        Path to timestamps numpy file (.npy), expected in nanoseconds.
    sph_coords_npz_path : str
        Path to spherical coordinates numpy archive (.npz) containing
        arrays named 'r', 'theta', and 'phi'.

    Returns:
    --------
    pd.DataFrame
        Copy of input dataframe with new columns:
        - 'R_SC'
        - 'Theta_SC'
        - 'Phi_SC'
    """
    timestamps = np.load(timestamps_npy_path)
    sph_coords = np.load(sph_coords_npz_path)

    if 'r' not in sph_coords or 'theta' not in sph_coords or 'phi' not in sph_coords:
        raise KeyError("sph_coords file must contain keys: 'r', 'theta', 'phi'")

    r_sph = sph_coords['r']
    theta_sph = sph_coords['theta']
    phi_sph = sph_coords['phi']

    if not (len(timestamps) == len(r_sph) == len(theta_sph) == len(phi_sph)):
        raise ValueError(
            "Length mismatch: timestamps and spherical coordinate arrays must have identical length."
        )

    t_vdf = pd.to_datetime(timestamps, unit='ns', utc=True)
    ...
    if isinstance(moments_df, list):
        out = pd.DataFrame(moments_df)
    elif isinstance(moments_df, pd.DataFrame):
        out = moments_df.copy()
    else:
        raise TypeError("moments_df must be a pandas DataFrame or a list of dict rows.")

    if 'Time' in out.columns:
        out['Time'] = pd.to_datetime(out['Time'], utc=True)
    else:
        if not isinstance(out.index, pd.DatetimeIndex):
            raise ValueError("moments_df must contain a 'Time' column or have a DatetimeIndex.")
        out['Time'] = pd.to_datetime(out.index, utc=True)

    out['Time_sec'] = out['Time'].dt.floor('s')

    sc_lookup['Time_sec'] = sc_lookup['Time_vdf'].dt.floor('s')
    sc_lookup = sc_lookup.sort_values('Time_vdf').drop_duplicates('Time_sec', keep='first')

    out = out.join(
        sc_lookup.set_index('Time_sec')[['R_SC', 'Theta_SC', 'Phi_SC']],
        on='Time_sec',
        how='left'
    )

    out = out.drop(columns=['Time_sec'])
...
def load_vdf(filename, index, VDF_SHAPE=(11, 9, 96)):
