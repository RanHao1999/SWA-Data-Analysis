...
def _prepare_gmm_data(V_para, V_perp1, V_perp2, vdf_corrected, n_component=3):
    ...
    # KMeans for weight initialisation (default n_init=10 for robustness)
...
def all_process(idx_time, initial_means):
    ...
    n_component = 3
    # Pre-compute X matrix and KMeans weights ONCE, shared across all covariance types.
    try:
        X, non_zero_idx, gmm_weights = _prepare_gmm_data(
            V_para, V_perp1, V_perp2, vdf_2ndcorrected, n_component)
    except Exception as e:
        print(f"[SKIP] {tslice_vdf} data prep failed: {e}")
        return initial_means

    # Try warm-start first; fall back to sklearn kmeans init if GMM collapses.
    # The block-averaged initial_means can be far from the true solution for
    # some slices, causing component collapse ("ill-defined empirical covariance").
    fitted = False
    for attempt, (init_means, label) in enumerate([
        (initial_means, "warm-start"),
        (None,           "kmeans-fallback"),
    ]):
        try:
            f_diag, dist_paras_diag, probas_diag = _cal_GMM_fast(
                X, non_zero_idx, vdf_2ndcorrected, 'diag', init_means, gmm_weights, n_component)
            f_alpha_diag, f_beam_diag, f_core_diag = f_diag
            f_spherical, dist_paras_spherical, probas_spherical = _cal_GMM_fast(
                X, non_zero_idx, vdf_2ndcorrected, 'spherical', init_means, gmm_weights, n_component)
            f_alpha_spherical, f_beam_spherical, f_core_spherical = f_spherical
            f_tied, dist_paras_tied, probas_tied = _cal_GMM_fast(
                X, non_zero_idx, vdf_2ndcorrected, 'tied', init_means, gmm_weights, n_component)
            f_alpha_tied, f_beam_tied, f_core_tied = f_tied
            fitted = True
            if attempt == 1:
                print(f"  [RETRY] {tslice_vdf} succeeded with kmeans fallback.")
            break
        except Exception as e:
            if attempt == 0:
                print(f"  [WARN] {tslice_vdf} warm-start failed: {e}")
            else:
                print(f"[SKIP] {tslice_vdf} GMM failed after fallback: {e}")
                return initial_means
...
def main():
    ...
    dt_seconds = 2.0  # PAS nominal cadence is 4 s — must match data resolution
    ...
    t_start = datetime(2024, 1, 8, 6, 0, 0)
